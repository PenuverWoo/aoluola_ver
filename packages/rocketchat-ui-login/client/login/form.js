...
Template.loginForm.events({
	'submit #login-card'(event, instance) {
		...
		if(state=='register'){
			var checked = event.target.roles.value;
			if(checked == 'doc'){
				console.log(Meteor.user);

				modal.open({
					title: 'WARNING',
					text:'Please contact admin : xxxxxxxx@gmail.com, submit doctor certification',
					type:'',
			...
			if (state === 'register') {
				...
				return Meteor[loginMethod](s.trim(formData.emailOrUsername), formData.pass, function(error) {
					...
					if(user.roles[0]=='user'){
						window.open('./direct/shaoyu.hu?layout=embedded','_self');
					}
...
Template.loginForm.onCreated(function() {
	...
	this.validate = function() {
		...
		//if the field got an invalud value
		if (state === 'register') {
